...
// Extract and parse enrollment data from an SF9/SF10 document without writing to the database.
// Records are gated to Grade 7-12.
exports.extractEnrollmentData = async ({ file, documentType = '', manual = false }) => {
    ...
    // 2. Extract text from PDF, Image, or Excel
    let text = '';
    try {
    ...
    // 3. Parse text using SF10 or SF9 parser
    ...
    // 4. Gather all records to enroll (use deduplicated scholasticRecords or fallback to single extracted)
    ...
    // Deduplicate: if 2 sem is the same, do not duplicate. Only Grade 7-12 records are accepted.
    const uniqueRecords = [];
    const seenSemKey = new Set();
    for (const rec of recordsToEnroll) {
        const gradeNum = parseInt(String(rec.gradeLevel).replace(/\D/g, ''), 10);
        if (isNaN(gradeNum) || gradeNum < 7 || gradeNum > 12 || !rec.section) continue;
    ...
    if (uniqueRecords.length === 0) {
        return {
            ok: false,
            message: `Could not extract a Grade 7-12 enrollment from ${docLabel}. Found -> Grade: "${gradeLevel || ''}", Section: "${section || ''}"`
        };
    }

    return { ok: true, isSF10, isSF9, docLabel, extracted, uniqueRecords };
};
...
exports.autoEnrollFromSF = async ({ studentId, file, documentType = '', userId = null, manual = false }) => {
    ...
    // 2. Extract + parse records (Grade 7-12 only, no writes here)
    const data = await exports.extractEnrollmentData({ file, documentType, manual });
    if (!data.ok) {
        return {
            success: false,
    ...
    const { docLabel, extracted, uniqueRecords } = data;
    const { schoolYear, gradeLevel, section, trackStrand } = extracted;
